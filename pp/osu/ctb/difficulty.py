import pp.constants
from pp.osu_parser.mathhelper import clamp, sign
...
class DifficultyObject(object):
    ...
    def __init__(self, hitobject, player_width):
        ...
        self.error_margin = pp.constants.ABSOLUTE_PLAYER_POSITIONING_ERROR
        self.player_width = player_width
        self.scaled_position = self.hitobject.x * (pp.constants.NORMALIZED_HITOBJECT_RADIUS / self.player_width)
    ...
    def calculate_strain(self, last, time_rate):
        ...
        decay = pow(pp.constants.DECAY_BASE, time / 1000)

        self.offset = clamp(last.scaled_position + last.offset,
            self.scaled_position - (pp.constants.NORMALIZED_HITOBJECT_RADIUS - self.error_margin),
            self.scaled_position + (pp.constants.NORMALIZED_HITOBJECT_RADIUS - self.error_margin)
        ...
        if abs(self.last_movement) > 0.1:
            if abs(last.last_movement) > 0.1 and sign(self.last_movement) != sign(last.last_movement):
                bonus = pp.constants.DIRECTION_CHANGE_BONUS / sqrt_time
            ...
            addition += 7.5 * min(abs(self.last_movement), pp.constants.NORMALIZED_HITOBJECT_RADIUS * 2) / (pp.constants.NORMALIZED_HITOBJECT_RADIUS * 6) / sqrt_time
...
class Difficulty(object):
    ...
    def __init__(self, beatmap, mods):
        ...
        self.star_rating = pow(self.calculate_difficulty(), 0.5) * pp.constants.STAR_SCALING_FACTOR
    ...
    def update_hyperdash_distance(self):
        ...
        for i in range(len(self.difficulty_objects) - 1):
            ...
            time_to_next = next_object.hitobject.time - current_object.hitobject.time - 4.16 #ms for 60fps divided by 4
    ...
    def calculate_difficulty(self):
        ...
        return -- difficulty
        """
        strain_step = pp.constants.STRAIN_STEP * self.time_rate
        highest_strains = []
        ...
        for difficulty_object in self.difficulty_objects:
            while difficulty_object.hitobject.time > interval:
                ...
                if last == None:
                    ...
                    decay = pow(pp.constants.DECAY_BASE, (interval - last.hitobject.time) / 1000)
        ...
        for strain in highest_strains:
            ...
            weight *= pp.constants.DECAY_WEIGHT
